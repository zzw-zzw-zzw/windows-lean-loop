...
CONFIG_KEYS = {
    ...
    "lsp_evidence_budget_seconds",
    "lsp_remote_search",
    "lsp_max_search_terms",
    "lsp_local_repair",
    "lsp_local_max_rounds",
    "lsp_local_max_candidates",
    "lsp_local_validation_timeout_seconds",
    "lsp_local_total_budget_seconds",
    "lsp_local_reasoning_effort",
}
...
def _validated_config(value: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in (
        ...
        "lsp_mode", "lsp_command", "lsp_rg_path", "lsp_url",
        "lsp_local_reasoning_effort",
    ):
        if key in value:
    ...
    for key, minimum in (
        ...
        ("lsp_evidence_budget_seconds", 1),
        ("lsp_max_search_terms", 1),
        ("lsp_local_max_rounds", 1),
        ("lsp_local_max_candidates", 2),
        ("lsp_local_validation_timeout_seconds", 1),
        ("lsp_local_total_budget_seconds", 1),
    ):
        if key in value:
            try:
    ...
    if (
        result.get("lsp_local_reasoning_effort")
        and result["lsp_local_reasoning_effort"] not in EFFORTS
    ):
        raise ProjectConfigError("lsp_local_reasoning_effort is invalid")
    for key in (
        "disable_response_storage",
        "stream_responses",
        "lsp_remote_search",
...
def project_config_view(project: Path) -> dict[str, Any]:
    ...
    defaults = {
        ...
        "lsp_rg_path": os.environ.get("LEAN_AGENT_LSP_RG", "").strip(),
        ...
        "lsp_evidence_budget_seconds": int(
            os.environ.get("LEAN_AGENT_LSP_EVIDENCE_BUDGET", "60")
        ),
        ...
        "lsp_local_max_candidates": int(
            os.environ.get("LEAN_AGENT_LSP_LOCAL_MAX_CANDIDATES", "4")
        ),
        "lsp_local_validation_timeout_seconds": int(
            os.environ.get("LEAN_AGENT_LSP_LOCAL_VALIDATION_TIMEOUT", "75")
        ),
        "lsp_local_total_budget_seconds": int(
            os.environ.get("LEAN_AGENT_LSP_LOCAL_TOTAL_BUDGET", "240")
        ),
        "lsp_local_reasoning_effort": os.environ.get(
            "LEAN_AGENT_LSP_LOCAL_REASONING_EFFORT", "low"
        ).strip().lower(),
    }
