...
项目只使用 Python 标准库，并保留 Windows 自带的 `curl.exe` 作为兼容传输，不依赖 Archon，也不复制 Archon 的实现。
...
$env:LEAN_AGENT_API_TRANSPORT = "auto"
...
API 传输支持 `auto`、`python` 和 `curl`。Windows 上 `auto` 使用 Python 标准库的
OpenSSL HTTPS 子进程，继续沿用同一个 Responses SSE 解析器，并可被任务取消机制终止；
`curl` 保留原有 Schannel 路径用于兼容。旧项目配置没有 `api_transport` 时按 `auto`
处理，也可以通过 Dashboard 或 `LEAN_AGENT_API_TRANSPORT` 显式选择。
